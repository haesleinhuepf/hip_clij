...
for image_size = 1000:500:8000
    ...
    %image_size = 1000;
    input_image = zeros(image_size, image_size);
    input_image(image_size / 2, image_size / 2) = 1000000;
    ...
    % test HIP
    hip_time_start = now;
    hip_smoothed_image = HIP.Gaussian(input_image, [sigma, sigma, 0], number_of_iterations, [ ] );
    % measure duration
    hip_duration = now - hip_time_start;

    % show result
    subplot(1,3,1), imshow(hip_smoothed_image(:,:,1), [0 255]);
    ...
    clijx.blur3D(clij_input_image, clij_smoothed_image, sigma, sigma, 0);
    ...
    % show result
    subplot(1,3,2), imshow(clij_result(:,:,1), [0 255]);
    ...
    subplot(1,3,3), imshow(difference(:,:,1));
